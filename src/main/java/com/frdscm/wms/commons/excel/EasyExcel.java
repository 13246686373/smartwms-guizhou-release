...
public final class EasyExcel implements Closeable {
     ...
     * 缓存类信息，提高性能
     */
    private static final ConcurrentMap<String, List<ExcelFiledInfo>> FILED_INFO = new ConcurrentHashMap<>();
    /**
     * spring的类型转换
     */
    ...
    private static final String DEFAULT_BOOL_FORMAT = "是:否";
     ...
     * @throws InvalidFormatException 非法的格式异常
     */
    private EasyExcel(InputStream inputStream) throws IOException, InvalidFormatException {
        this.startRow = 0;
        this.sheetName = "Sheet1";
        this.excelFilePath = "";
    ...
    public <T> List<T> parse(Class<T> clazz) {
        ...
        try {
            ...
            if (null != sheet) {
                ...
                Map<String, ExcelFiledInfo> infoMap = new HashMap<>(0);
                ...
                Map<String, String> titleMap = new HashMap<>(0);
                for (Cell title : row) {
            ...
            throw new BusinessException("初始化异常");
        } catch (ParseException e) {
            logger.error("时间格式化异常", e);
            throw new BusinessException("时间格式化异常");
        } catch (Exception e) {
            logger.error("其他异常", e);
    ...
    public <T> boolean createExcel(List<T> list) {
        ...
        try {
            ...
            createTitleRow(infoList, titleRow, fieldSize);
            // 生成数据行
            for (int i = 0, length = list.size(); i < length; i++) {
                ...
                for (int j = 0; j < fieldSize; j++) {
                    ...
                    setExcelCellValue(cell, info, format, object);
                }
            }
            File file = new File(this.excelFilePath);
            if (!file.exists()) {
    ...
    public <T> Workbook reportExcel(List<T> list) {
        // 获取bean的配置信息
        T test = list.get(0);
        List<ExcelFiledInfo> infoList = getFiledInfo(test.getClass());
        // 排序
    ...
    private void createTitleRow(List<ExcelFiledInfo> infoList, Row titleRow, int fieldSize) {
    ...
    private void setExcelCellValue(Cell cell, ExcelFiledInfo info, String format, Object object) {
        CellStyle cellStyle;
        switch (info.getType()) {
            case TEXT:
    ...
    private List<ExcelFiledInfo> loadFiledInfo(Class<?> clazz) {
        List<ExcelFiledInfo> infoList = new ArrayList<>();
